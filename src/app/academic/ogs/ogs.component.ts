import { Component, OnInit } from '@angular/core';
import {Http, Headers, RequestOptions} from '@angular/http';
import { GlobalService } from './../../global.service';
import {MatDialog, MatDialogRef, MAT_DIALOG_DATA} from '@angular/material';

import Swal from 'sweetalert2';
const swal = Swal;
import { map } from 'rxjs/operators';
import * as pdfMake from "pdfmake/build/pdfmake";
import * as pdfFonts from 'pdfmake/build/vfs_fonts';
(<any>pdfMake).vfs = pdfFonts.pdfMake.vfs;

@Component({
  selector: 'app-ogs',
  templateUrl: './ogs.component.html',
  styleUrls: ['./ogs.component.css']
})
export class OGSComponent implements OnInit {
  codesummaryArr=[]
  classlistArr=[]

  CNum = null;
  newCodeListArr=[]

   ogsid=0
   ogsstatus =''
   ogsstatustext=''
   academicdean=''

   repostbut=false
  constructor(public global: GlobalService,private http: Http,) {
    this.http.get(this.global.api+'Admission/SYSettings/'+this.global.syear,this.global.option)
      .map(response => response.json())
      .subscribe(res => {
        if(res.data.gradeEntry==1){
          this.repostbut=true
        }
        console.log(res)
      })

  }

 keyDownFunction(event){
  if(event.keyCode == 13 || event.keyCode == 9 || event == 'onoutfocus') {
    this.getpurglist()
   }
}
errs=''
getpurglist(){
  this.global.swalLoading('')
  this.classlistArr=undefined

          this.getCodeSummary()
  
}
  ngOnInit() {
  this.errs='start'
  }
  status
  checkremarks()
  {
    this.http.get(this.global.api+'OnlineGradingSheet/Status/'+this.ogsid,this.global.option)
      .map(response => response.json())
      .subscribe(res => {
          this.ogsstatus = res.data[0].statusDesc
          this.ogsstatustext = res.data[0].statusDesc
          this.status = res.data
          this.global.swalClose()
      },Error=>{
        this.global.swalAlertError();
      });
  }
  repost=false
  getCodeSummary(){
    this.codesummaryArr=[]
    this.ogsstatus=''
    this.http.get(this.global.api+'EmployeePortal/CodeSummary/'+this.global.syear+'?codeNo='+this.CNum.toUpperCase(),this.global.option)
        .map(response => response.json())
        .subscribe(res => {
            this.ogsstatus='Not yet downloaded';
            this.codesummaryArr=res.data;   
            this.http.get(this.global.api+'OnlineGradingSheet/'+this.CNum.toUpperCase()+'/'+this.global.syear,this.global.option)
                .map(response => response.json())
                .subscribe(res => {
                  //this.global.swalClose()
                  if (res.data.length!=0) {
                    this.ogsid = res.data[0].id

                    this.academicdean = res.data[0].academicHead
                    this.repost = false
                    this.http.get(this.global.api+'OnlineGradingSheet/Grades/'+this.CNum.toUpperCase()+'/'+this.global.syear,this.global.option)
                        .map(response => response.json())
                        .subscribe(res => {
                            console.log(res)
                           for (var i = 0; i < res.data.length; ++i) {
                             if(res.data[i].acadHistGradePosted==''){
                               this.repost = true
                             }
                           }
                            this.classlistArr = res.data
                            this.checkremarks()
                      //       this.http.post(this.global.api+'OnlineGradingSheet/Status',{
                      //   "OGSheetId": this.ogsid,
                      //   "StatusId": 1,
                      //   "EmployeeId": this.global.requestid(),
                      //   "Remarks": ""
                      // },this.global.option)
                      // .map(response => response.json())
                      // .subscribe(res => {
                        
                      //   },Error=>{
                      //    this.global.swalAlertError();
                      // });
                        },Error=>{
                          this.global.swalAlertError();
                        });

                    this.errs=''
                  }else{
                    this.global.swalClose()
                    this.errs='error'
                  }
                  
                },Error=>{ 
                  this.global.swalAlertError(); 
                });  
          },Error=>{
                  //console.log(Error);
                  this.global.swalAlertError();
                  //console.log(Error)
        });
  }
 getpercentage(y,temp){
   if (this.classlistArr!=undefined) {
     var x=0;
     for (var i = 0; i < this.classlistArr.length; ++i) {
       if(this.classlistArr[i].remarks==temp){
         x++;
       }
     }
     if (y==0) {
       return x
     }
     var rets 
     rets = (x/this.classlistArr.length)  * 100
     rets=Math.round(rets * 10) / 10
     return rets
   }
   return ''
   
 }

 swalConfirm()
  {
    var x = ''
    if(this.ogsstatus == "Not Yet Uploaded"){
      this.ogsstatustext = this.ogsstatus;
      x=x+"*Unable to updated status! Please remind the fuculty to Upload in MyPortal.<br>"
    }
    if(this.ogsstatus == "Posted"){
      x=x+"*Unable to updated status! Once Posted, status cannot be updated.<br>"
    }

    if (x=='') {
      var textcheck2=''
        textcheck2 == "You can't revert this action!"
      swal.fire({
          title: 'Change status confirmation!',
          html: 'Are you sure you want to Change the Status? '+textcheck2,
          type: 'info',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
          cancelButtonText: 'No',
          confirmButtonText: "yes"
        }).then((result) => {
          if (result.value) {
            var y=0
            if (this.ogsstatustext=='Not Yet Uploaded') {
              y = 0
            }
            if (this.ogsstatustext=='Posted') {
              y = 2
            }
              this.http.post(this.global.api+'OnlineGradingSheet/Status',{
                  "OGSheetId": this.ogsid,
                  "StatusId": y,
                  "EmployeeId": this.global.requestid(),
                  "Remarks": ""
                },this.global.option)
                .map(response => response.json())
                .subscribe(res => {
                  if (this.ogsstatustext=='Posted') {
                    this.postgrades()
                  }else
                  this.keyDownFunction('onoutfocus')
                  },Error=>{
                   this.global.swalAlertError();
                });
          }else{
            this.ogsstatustext = this.ogsstatus
          }
        })
    }else{
      setTimeout(function(){ this.ogsstatustext = this.ogsstatus; }, 3000);
      this.global.swalAlert("Status update failed!",x,'warning')
    }
  }

  postgrades(){
    var length = this.classlistArr.length-1
    var arr=[]
      for (var i = 0; i < this.classlistArr.length; ++i) {
       arr.push(
         {
          "IdNumber": this.classlistArr[i].idNumber,
          "SchoolYear": this.global.syear,
          "Grade": this.classlistArr[i].grade,
          "RecordId":this.classlistArr[i].acadHistRecordID
        })
      }
      this.http.put(this.global.api+'OnlineGradingSheet/ClassEGS/'+this.CNum,arr
        ,this.global.option)
        .map(response => response.json())
        .subscribe(res => {  
          this.keyDownFunction('onoutfocus')
        },Error=>{
           this.global.swalAlertError();
        });     
  }

  generatePDF(){
    //place this block of codes inside a function
var SY = this.global.syDisplay(this.global.syear);
var SubjectID = this.codesummaryArr[0].subjectId;
var SubjectDesc = this.codesummaryArr[0].subjectTitle
var CodeNo = this.CNum;
var LecUnits = this.codesummaryArr[0].lecUnits;
var LabUnits = this.codesummaryArr[0].labUnits;

var Passed = this.getpercentage(0,'Passed');
var Failed = this.getpercentage(0,'Failed');
var Dropped = this.getpercentage(0,'Dropped');
var NFE = this.getpercentage(0,'No Final Exam');
var WP = this.getpercentage(0,'Widrawn with Permission');
var BlankGrade = this.getpercentage(0,'Blank Grade'); 
var total = Passed+Failed+Dropped+NFE+WP+BlankGrade;

var PassedPer = this.getpercentage(1,'Passed');
var FailedPer =this.getpercentage(1,'Failed');
var DroppedPer =this.getpercentage(1,'Dropped');
var NFEPer = this.getpercentage(1,'No Final Exam');
var WPPer = this.getpercentage(1,'Widrawn with Permission');
var BlankGradePer = this.getpercentage(1,'Blank Grade');;
var totalPer = Math.round(PassedPer+FailedPer+DroppedPer+NFEPer+WPPer+BlankGradePer);

var Instructor = this.codesummaryArr[0].instructor;
var Dean = this.academicdean;


 var headers = [
        {text:'No.',bold:true},
        {text:'IDNUMBER',bold:true,alignment:'center'},
        {text:'NAME',bold:true,alignment:'center'},
        {text:'GENDER',bold:true,alignment:'center'},
        {text:'GRADE',bold:true,alignment:'center'},
        {text:'REMARKS',bold:true,alignment:'center'},
        ];
    var tableData = [];
    var res = [];

    res.push(headers);
    for(var x = 0;x<this.classlistArr.length;x++){
        tableData = [];
                            if (this.classlistArr[x].remark=='Widrawn with Permission') {
                       this.classlistArr[x].remark = "WP"
                    }
                    if (this.classlistArr[x].remark=='No Final Exam') {
                       this.classlistArr[x].remark = "NFE"
                    }
        tableData.push(
            {text:x+1,alignment:'right'},
            {text:this.classlistArr[x].idNumber,alignment:'center'},
            {text:this.classlistArr[x].name},
            {text:this.classlistArr[x].gender,alignment:'center'},
            {text:this.classlistArr[x].grade,alignment:'center'},
            this.getstyle(x)
        );
        res.push(tableData); 
    }
var dd = {
    pageSize:'LEGAL',
  content: [
    {text:'University of Saint Louis\nTuguegarao City, Cagayan\nFINAL GRADES\n\n',bold:true,fontSize:10,alignment:'center'},{
          image: 'data:image/png;base64,', 
          width: 200,
          opacity: 0.1,
          absolutePosition: { x: 210, y: 370 },
        },
    {
        stack:
        [
            {
                table:{
                    widths:[85,250,45,45,45],
                    body:
                    [
                        [
                            {text:'School Year:',bold:true},
                            {text:SY,colSpan:2},                        //SchoolYear Variable
                            {},
                            {text:'Code No:',bold:true},
                            {text:CodeNo,alignment:'center'}            //Code Number Variable
                        ],
                        [
                            {text:'Subject ID:',bold:true},
                            {text:SubjectID,colSpan:2},                 //Subject ID Variable
                            {},
                            {text:'Lec. Units:',bold:true},
                            {text:LecUnits,alignment:'center'}          //Lecture Units Variable
                        ],
                        [
                            {text:'Subject Description:',bold:true},
                            {text:SubjectDesc,colSpan:2},               //Subject Description Variable
                            {},
                            {text:'Lab. Units:',bold:true},
                            {text:LabUnits,alignment:'center'}          //Labaratory Units Variable
                        ]
                    ]
              },fontSize:9,
              layout:{
                      hLineWidth: function (i, node) { return 0.75; },
                        vLineWidth: function (i, node) { return 0.75; },
                        paddingTop: function(i, node) { return 1; },
                        paddingBottom: function(i, node) { return 1; },
                  } 
            },
            {text:'\n',fontSize:5},
            {
                table:{
                    widths:[250,80,50,45,45],
                    body:[
                        [
                            {text:'',border:[]},
                            {text:'',border:[]},
                            {text:'Remarks',alignment:'center',border:[]},
                            {text:'Total',alignment:'center',border:[]},
                            {text:'%',alignment:'center',border:[]},
                        ],
                        [
                            {text:'Instructor:',bold: true,rowSpan:2,border:[true,true,true,false]},
                            {text:'Date Signed',bold: true,alignment:'center',rowSpan:3},
                            {text:'Passed',alignment:'center'},
                            {text:Passed,alignment:'center'},           ////////////////Passed Variable
                            {text:PassedPer+'%',alignment:'center'},    ////////////////Passed Percentage Variable
                        ],
                        [
                            {},
                            {},
                            {text:'Failed',alignment:'center'},
                            {text:Failed,alignment:'center'},           ////////////////Variable
                            {text:FailedPer+'%',alignment:'center'},    //////////////// Percentage Variable
                        ],
                        [
                            {text:Instructor,alignment:'center',border:[true,false,true,true]},////////////////Variable
                            {},
                            {text:'Dropped',alignment:'center'}, 
                            {text:Dropped,alignment:'center'},           ////////////////Dropped Variable
                            {text:DroppedPer+'%',alignment:'center'},    ////////////////Dropped Percentage Variable
                        ],
                        [
                            {text:'Academic Dean:',bold: true,rowSpan:2,border:[true,true,true,false]},////////////////Variable
                            {text:'Date Signed',bold: true,alignment:'center',rowSpan:3},
                            {text:'NFE',alignment:'center'},
                            {text:NFE,alignment:'center'},           ////////////////NFEVariable
                            {text:NFEPer+'%',alignment:'center'},    ////////////////NFE Percentage Variable
                        ],
                        [
                            {},
                            {},
                            {text:'WP',alignment:'center'},
                            {text:WP,alignment:'center'},           ////////////////WP Variable
                            {text:WPPer+'%',alignment:'center'},    ////////////////WP Percentage Variable
                        ],
                        [
                            {text:Dean,alignment:'center',border:[true,false,true,true]},////////////////Variable
                            {},
                            {text:'Blank Grade',alignment:'center'},
                            {text:BlankGrade,alignment:'center'},           ////////////////Blank Grade Variable
                            {text:BlankGradePer+'%',alignment:'center'},    ////////////////Blank Grade Percentage Variable
                        ],
                        [
                            {text:'',border:[false,true,false,false]},
                            {text:'',border:[false,true,false,false]},
                            {text:'Total',border:[false,true,false,false],alignment:'center',bold: true},
                            {text:total,border:[false,true,false,false],alignment:'center',bold: true},////////////////Variable
                            {text:totalPer+'%',border:[false,true,false,false],alignment:'center',bold: true}////////////////Variable
                        ]
                    ]
                },fontSize:9,
                  layout:{
                          hLineWidth: function (i, node) { return 0.75; },
                            vLineWidth: function (i, node) { return 0.75; },
                            paddingTop: function(i, node) { return 1; },
                            paddingBottom: function(i, node) { return 1; },
                      }
            }
        ]
    },
    {text:'*Grade Legend: D is Dropped; N is NFE; WP is for Withdrawn with Permission',italics:true,fontSize:7},
    {
        table:{
            headerRows: 1,
            pageBreak:'before',
            dontBreakRows: true,
            widths:[20,47,260,45,40,50],
            body:res,
        },fontSize:9,
        layout:{
                  hLineWidth: function (i, node) { return 0.75; },
                  vLineWidth: function (i, node) { return 0.75; },
                  paddingTop: function(i, node) { return 1; },
                  paddingBottom: function(i, node) { return 1; },
              }
    },
    {text:this.slashn(1)},
    {text:'Checked by:\n'+this.slashn(2)+'\nName and Signature',fontSize:9}
  ],
  footer:function(currentPage, pageCount, pageSize) {
      return [
          {text:currentPage+' of '+pageCount,alignment:'right',fontSize:9,margin:[0,0,40,0]}
      ]
  }
  
}
pdfMake.createPdf(dd).open();
  }
slashn(x){
  if(x==1){
    if(this.classlistArr.length>48){
      return ''
    }
    return '\n'
  }
  if(x==2){
    if(this.classlistArr.length<=47){
      return '\n\n'
    }
    if(this.classlistArr.length==48){
      return '\n'
    }
    if(this.classlistArr.length>49){
      return ''
    }
  }
}
getremark(i){
  if(this.classlistArr[i].grade>74&&this.classlistArr[i].grade<101){
    return "Passed"
  }else 
  if(this.classlistArr[i].grade==70){
    return "Failed"
  }else 
  if(this.classlistArr[i].grade=='D'){
    return "Dropped"
  }else 
  if(this.classlistArr[i].grade=='N'){
    return "No Final Exam"
  }else 
  if(this.classlistArr[i].grade=='WP'){
    return "Widrawn with Permission"
  }else 
  if(this.classlistArr[i].grade==''||this.classlistArr[i].grade==null){
    return "Blank Grade"
  }else{
   return "Invalid Input"
  }

}


getstyle(x){
  if(this.getremark(x)=='Widrawn with Permission'){
    return {text:this.getremark(x),alignment:'center',fontSize:5}
  }
  return {text:this.getremark(x),alignment:'center'}
}
}

